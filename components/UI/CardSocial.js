...
const CardSocail = (props) => {
    const { post, userId, fromUserProfile } = props;

    console.log('POST: ', post._links["wp:featuredmedia"][0].href)
            ...
            <View style={styles.card}>
                ...
                <View style={styles.cardImageContainer} >
                    <Image 
                        ...
                        // source={{ uri: post._links["wp:featuredmedia"][0].href}}
                        source={{ uri: 'http://go.bestfaretravelo.com/wp-content/uploads/2016/12/photo-1469920783271-4ee08a94d42d.jpg'}}
                    ...
                    {/* {item._embedded['wp:featuredmedia'].filter( 
                        element=>element.id == item.featured_media).map((subitem, index) => (
                            ...
                            style={{height:200, width:200, flex:1}}key = {item.id}
                            />
                    ))} */}

                    {/* <Text>{post['wp:featuredmedia'][0].href}</Text> */}
                    {/* <ActivityIndicator 
                ...
                <View style={styles.cardHeader}>
                    <View>
                        <Text style={styles.title}>{post.title.rendered ? post.title.rendered : ""}</Text>
                        { post.body && post.content.rendered.length > 30 ? (
                            <View>
                                { showFullBody ? (
                                    ...
                                    <Text style={styles.description}> 
                                        {post.content.rendered.substring(0, 30)}
                                        <Text
                                            style={{ color: Colors.brightBlue }}
                                            onPress={() => setShowFullBody((prevState) => !prevState)} 
                                        >
                            ...
                            <Text style={styles.description}> {post.content.rendered} </Text>
                        ) }
